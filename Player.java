...
public class Player implements Collideable, KeyListener {
	...
	private final static int SHOOT_DELAY = 7;
	...
	public void step() {
		...
		if(keys[SHOOT]){
			if(shootTimer == 0) {
				shot = true;
				Hazards.addHazard(new PlayerBullet(xPos-100*Math.cos(Math.toRadians(playerDirection))+27*Math.sin((Math.toRadians(playerDirection))), yPos-100*Math.sin(Math.toRadians(playerDirection))-27*Math.cos(Math.toRadians(playerDirection)), playerDirection));
				shootTimer = 1;
